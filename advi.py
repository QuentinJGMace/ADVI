...
class ADVI:
    ...
    def init_meanfield(self):
        self.model_params = ModelParam(self.advi_dim, mode="meanfield")
    ...
    def compute_elbo(self, batch, full_data_size, batch_indices=None):
        elbo = 0
        for i in range(self.num_samples):
            sample = torch.randn(self.advi_dim)
            zeta = self.zeta_from_sample(sample)  
            theta = self.model.theta_from_zeta(zeta)
            elbo += self.model.log_prob(batch, theta, full_data_size, batch_indices) + self.model.log_det(zeta) + self.entropy()
    ...
    def fit(self, x, method = "SGD", plotting=True, use_grad_clip=True):
        ...
        while counter < self.num_epochs and diff_elbo > self.delta_elbo_threshold:
            ...
            for i_batch in range(0, x.size(0), self.batch_size):
                ...
                optimizer.zero_grad()
                elbo = self.compute_elbo(batch, x.shape[0], batch_indices)
                loss = -elbo
                loss.backward()
                if use_grad_clip:
            ...
            try:
                with torch.no_grad():
                    elbo = self.compute_elbo(x, x.shape[0], np.arange(x.shape[0]))
